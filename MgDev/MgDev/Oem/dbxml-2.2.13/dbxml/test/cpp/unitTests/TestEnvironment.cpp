...
//
// $Id: TestEnvironment.cpp,v 1.24 2005/10/11 16:33:10 gmf Exp $
//
...
namespace {
	...
	TestEnvironment::TestType convertTestType(const std::string &type)
	{
		...
		else if(!type.compare("updateDocument"))
			t = TestEnvironment::UPDATE_DOCUMENT;
	...
	XmlQueryContext::ReturnType convertReturnType(const std::string &str)
	{
		...
		if(!str.compare("DeadValues"))
			rt = XmlQueryContext::DeadValues;
		else if(!str.compare("LiveValues"))
			rt = XmlQueryContext::LiveValues;
...
TestEnvironment::TestEnvironment(TestLogger *log, const std::string &db_home,
	...
	: log_(log),
	  ...
	  flags_(indexNodes ? DBXML_INDEX_NODES : 0),
	  secure_(secure),
	  validate_(validate)
{
	if(debug) {
	...
	xqContext_ = new (memoryManager_) XQContextImpl(memoryManager_);
			  ...
			  const Iterator<std::string> &args)
{
	...
	case TestEnvironment::QUERY_PLAN:
		...
		break;
	case TestEnvironment::UPDATE_DOCUMENT:
		test = new UpdateDocumentTest(*this, args);
		break;
	default:
	...
	const std::string &query, DOMNode *contextNode)
{
	...
	XQQuery *parsedQuery = XQEvaluator::parse(UTF8ToXMLCh(query).str(),
						  xqContext_);

	// set node context and evaluate query
	xqContext_->setExternalContextNode(n);
	xqContext_->setContextPosition(1);
	xqContext_->setContextSize(1);
	Sequence result = parsedQuery->
		evaluate(xqContext_).toSequence(xqContext_);
	return result;
}
...
void TestEnvironment::addIndexSpecifications(Sequence &result)
{
	...
	for (Sequence::iterator it = result.begin(); it != result.end(); it++) {
		if((*it)->isNode()) {
			...
			getIndexSpecification(nodeImpl->getDOMNode(),
					      uri, node, index);
...
void TestEnvironment::addVariables(Sequence &result)
{
	...
	for (Sequence::iterator it = result.begin(); it != result.end(); it++) {
		if((*it)->isNode()) {
			...
			string name, value;
			getVariable(nodeImpl->getDOMNode(), name, value);
					   ...
					   std::string &text)
{
	...
	const NodeImpl *nodeImpl = (const NodeImpl*)theNode->getInterface(Item::gPathan);
	assert(nodeImpl != 0);

	const DOMNode *n = nodeImpl->getDOMNode();
	if(n->getFirstChild()) {
...
void TestEnvironment::createDBXMLEnvironment(const std::string &db_home)
{
	...
	int dberr = 0;
	u_int32_t dbFlags = (DB_CREATE|DB_INIT_LOCK|DB_INIT_LOG|DB_INIT_MPOOL);
	if(transacted_) dbFlags |= DB_INIT_TXN;
	...
	try {
		dberr = env->open(db_home.c_str(), DB_JOINENV, 0);
	} catch (DbException &de) {
		dberr = de.get_errno();
	}
	if (dberr != 0) {
		if(log_) ENV_MSG(*log_, "Creating new environment");
		dberr = env->open(db_home.c_str(), dbFlags, 0);
	} else {
		if(log_) ENV_MSG(*log_, "Joined existing environment");
	}
...
Sequence TestEnvironment::parseTestInstructions()
{
	...
	Sequence dseq = xqContext_->resolveDocument(ifile.str());

	const NodeImpl *nodeImpl = (const NodeImpl*)dseq.first()->getInterface(Item::gPathan);
	assert(nodeImpl != 0);

	DOMNode *dnode = (DOMNode*)nodeImpl->getDOMNode();
...
void TestEnvironment::addGroupContext()
{
	...
	const NodeImpl *nodeImpl = (const NodeImpl*)result.first()->getInterface(Item::gPathan);
	assert(nodeImpl != 0);

	const DOMNode *context = nodeImpl->getDOMNode();
