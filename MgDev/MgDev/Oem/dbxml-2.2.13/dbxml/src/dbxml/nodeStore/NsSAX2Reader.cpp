...
//
// $Id: NsSAX2Reader.cpp,v 1.25 2005/11/15 12:56:27 gmf Exp $
//
...
class NsEventAttrListXimpl : public NsEventAttrList {
public:
	...
	virtual const xmlch_t *qname(int index) {
		return (_list.elementAt(index))->getQName();
	}
	...
	virtual const int uriId(int index) const {
		...
		if (XPath2Utils::equals(attr->getQName(),
					XMLUni::fgXMLNSString))
			uriId = 4; // hard-coded XXX
		return uriId - DBXML_MIN_URI;
	}
...
NsSAX2Reader::NsSAX2Reader(XmlManager &mgr, Transaction *txn,
			   unsigned int flags,
...
void NsSAX2Reader::setExitOnFirstFatalError(const bool newState)
{
	fScanner->setExitOnFirstFatal(newState);
}

void NsSAX2Reader::setValidationConstraintFatal(const bool newState)
{
...
void NsSAX2Reader::parse (const InputSource& source)
{
	...
	try {
		...
		fParseInProgress = false;
		NsUtil::nsThrowException(XmlException::INTERNAL_ERROR,
					 (std::string("DOMException exception during NsSAX2Reader parse: ")
					 + XMLChToUTF8(e.getMessage()).str()).c_str(),
					 __FILE__, __LINE__);
	}
	catch (...) {
...
{
	uint32_t length = XMLString::stringLen(commentText);
...
void NsSAX2Reader::endEntityReference(const XMLEntityDecl& entityDecl)
{
	...
	uint32_t length = XMLString::stringLen(name);
...
void NsSAX2Reader::startElement(
	...
	const bool isRoot)
{
	NsEventAttrListXimpl alist(attrList, *this, attrCount);
	const XMLCh *uri = 0;
...
void NsSAX2Reader::endIntSubset()
{
	...
	uint32_t len = XMLString::stringLen(subset);
...
void NsSAX2Reader::setFeature(const XMLCh* const name, const bool value)
{
	NsUtil::nsThrowException(XmlException::INTERNAL_ERROR,
...
void NsSAX2Reader::setDoNamespaces(const bool newState)
{
...
void NsSAX2Reader::setDoSchema(const bool newState)
{
