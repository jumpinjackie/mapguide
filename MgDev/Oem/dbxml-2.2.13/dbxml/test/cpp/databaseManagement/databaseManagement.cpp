...
//
// $Id: databaseManagement.cpp,v 1.15 2005/10/12 15:53:01 gmf Exp $
//
...
int main(int argc, char **argv)
{
  ...
  try {
    ...
    INFO_MSG(log, "Caught Pathan Exception: %1") << XMLChToUTF8(e.getString()).str();
  }
  catch(DbException &de) {
    INFO_MSG(log, "Caught DB Exception: %1: %2") << de.get_errno() << (char*)de.what();
  }
	...
	const std::string &path2DbEnv1, const std::string &path2DbEnv2)
{
	...
	TEST_MSG(log, "8.5.1 - create 2 database environments using the same directory");
	{
		...
		DbEnv env2(0);
		env2.open(path2DbEnv1.c_str(), flags, 0);
		XmlManager db2(&env2);
		db2.setDefaultContainerType(nodeLevelStorage ?
					    XmlContainer::NodeContainer :
					    XmlContainer::WholedocContainer);
		db2.setDefaultContainerFlags(indexNodes ? DBXML_INDEX_NODES : 0);
		XmlContainer *c2;
		if(transacted) {
	...
	TEST_MSG(log, "8.5.2 - create 2 database environments using separate directories");
	{
		...
		DbEnv env2(0);
		env2.open(path2DbEnv2.c_str(), flags, 0);
		XmlManager db2(&env2);
		db2.setDefaultContainerType(nodeLevelStorage ?
					    XmlContainer::NodeContainer :
					    XmlContainer::WholedocContainer);
	...
	{
		try {
			// try to remove original env first, and ignore errors
			DbEnv tenv(0);
			tenv.remove(path2DbEnv1.c_str(), 0);
		} catch (...) {}
		DbEnv env(0);
		env.open(path2DbEnv1.c_str(), flags|DB_INIT_TXN, 0);
		XmlManager db1(&env);
		db1.setDefaultContainerType(nodeLevelStorage ?
					    XmlContainer::NodeContainer :
					    XmlContainer::WholedocContainer);
	...
	// a.  same as 8.5.3, but use externally created DbTxn, and
