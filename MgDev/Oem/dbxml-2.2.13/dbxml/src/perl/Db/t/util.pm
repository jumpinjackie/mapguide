...
sub joiner
{
    ...
    for ( my $status = $cursor->c_get($k, $v, Db::DB_FIRST) ;
          $status == 0 ;
          $status = $cursor->c_get($k, $v, Db::DB_NEXT)) {
	push @data, $v ;
    }
...
sub countRecords
{
   ...
   #for ($status = $cursor->c_get($k, $v, Db::DB_FIRST) ;
#	$status == 0 ;
#	$status = $cursor->c_get($k, $v, Db::DB_NEXT) )
   while ($cursor->c_get($k, $v, Db::DB_NEXT) == 0)
     { ++ $count }
...
{
    my $E ;
    if ($E = catch DbException)
    {
        my $ref = $E->type() ;
        print "Caught $ref " . $E->what() . "', errno is " . $E->get_errno . "\n" ;
        exit;
    }
    elsif ($E = catch std::exception)
    {
        print "Caught std::exception " . $E->what() . " \n" ;
        exit;
    }
    elsif ($@)
    {
