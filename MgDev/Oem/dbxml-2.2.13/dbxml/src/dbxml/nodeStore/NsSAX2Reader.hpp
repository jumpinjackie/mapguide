...
//
// $Id: NsSAX2Reader.hpp,v 1.13 2005/05/25 16:02:50 jsnelson Exp $
//
 ...
 * yet be removed.
 *
 * It communicates with BDB XML through the class NsEventHandler,
...
public :
	...
	void setNsEventHandler(NsEventHandler *handler) { fHandler = handler; }
	NsEventHandler *getNsEventHandler() const {
		return fHandler;
	}
 ...
 */
class NsParserEventSource : public NsPushEventSource
{
public:
	enum NsParserFlags {
		NS_PARSER_WELL_FORMED = 1,
		NS_PARSER_MUST_VALIDATE = 2
	};
	...
	virtual NsEventHandler *getNsEventHandler(void) const {
		return parser_.getNsEventHandler();
	}
	virtual void setNsEventHandler(NsEventHandler *handler) {
