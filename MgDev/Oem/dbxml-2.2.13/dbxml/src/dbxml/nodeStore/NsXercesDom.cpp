...
//
// $Id: NsXercesDom.cpp,v 1.39 2005/08/24 12:44:29 jsnelson Exp $
//
#include "NsUtil.hpp"
...
static inline NsDomNode *fakeDynamicCastNsDomNode(DOMNode *node)
{
	if (!node) return 0;
...
//
// DOMTypeInfo class to make Pathan happy
//
			       ...
			       int index, uint32_t type)
{
	...
	case NS_TEXT: {
		...
		(void) new (dt) NsXDOMText(parent, index, type);
		addToDomFreeList(dt);
		return dt;
	}
	case NS_COMMENT: {
		...
		if (!dt) NsXDOMNoMemory("createNsDomText");
		(void) new (dt) NsXDOMComment(parent, index);
		addToDomFreeList(dt);
		return dt;
	}
	case NS_PINST: {
		...
		if (!dt) NsXDOMNoMemory("createNsDomText");
		(void) new (dt) NsXDOMProcessingInstruction(parent, index);
		addToDomFreeList(dt);
		return dt;
	}
	default:
...
{
	if (getNsDocument()->getXmlDecl())
		return getNsDocument()->getXmlDecl();
	return _decl_1_0;
}
...
{
	return getNsDocument()->getEncodingStr();
}
...
NsXDOMDocument::appendChild(DOMNode *newChild)
{
	...
	return fakeDynamicCastDOMNode(insertNsChild(child, 0));
}
...
NsXDOMDocument::insertBefore(DOMNode *newChild, DOMNode *refChild)
{
	...
	return fakeDynamicCastDOMNode(insertNsChild(child, rchild));
}
...
{
	return ((const DOMNode *)this == other);
}

bool
NsXDOMDocument::isEqualNode(const DOMNode* arg) const
{
	const NsDomNode *thisNode = fakeDynamicCastNsDomNode(this);
...
_compareNsPositions(const DOMNode *node1, const DOMNode *node2)
{
	if(node1 == 0 || node2 == 0 ||
	   ...
	   (node2->getNodeType() == DOMNode::DOCUMENT_NODE ? node2 : node2->getOwnerDocument())) {
		return DOMNode::TREE_POSITION_DISCONNECTED;
	}

	short nsret = NsDomNode::compareNsPositions(
		fakeDynamicCastNsDomNode(node1),
		fakeDynamicCastNsDomNode(node2));
	return nsret;
}
...
{
	return (DOMDocument *) ((NsXDOMElement*)this)->getNsDocumentObj();
}
				     ...
				     bool useDefault) const
{
	...
	NsDomElement *thisElem = (NsDomElement*)fakeDynamicCastNsDomNode(this);
...
{
	return (DOMDocument *) (const_cast<NsXDOMAttr*>(this))->
		getNsDocumentObj();
}
...
NsXDOMAttr::lookupNamespacePrefix(const XMLCh* namespaceURI,
				  bool useDefault) const
{
	NsDomAttr *thisAttr = (NsDomAttr*)fakeDynamicCastNsDomNode(this);
...
{
	return (DOMDocument *) const_cast<NsXDOMText*>(this)->
		getNsDocumentObj();
}
...
{
	return (DOMDocument *) const_cast<NsXDOMComment*>(this)->
		getNsDocumentObj();
}
...
{
	return (DOMDocument *) const_cast<NsXDOMProcessingInstruction*>(this)->
		getNsDocumentObj();
}
...
{
	NsDomText *thisText = (NsDomText *)fakeDynamicCastNsDomNode(this);
	return compareNsNodes(thisText, (const NsDomNode *)
			      (((DOMNode*)arg)->getInterface(_nsDomString)));
}
