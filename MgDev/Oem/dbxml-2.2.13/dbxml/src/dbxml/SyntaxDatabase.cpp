...
//
// $Id: SyntaxDatabase.cpp,v 1.26 2005/11/22 16:39:49 jsnelson Exp $
//
			      ...
			      pageSize, flags & DB_XA_CREATE))
{
	...
	try {
		...
		// Open the Db objects
		err = index_->open(txn, /*duplicates*/true, nodesIndexed, flags, mode);
		if(err == 0)
			err = statistics_->open(txn, /*duplicates*/false, nodesIndexed, flags, mode);
				 ...
				 SecondaryDatabase::Operation op2, const Key &k2) const
{
	...
	if(err == 0) {
		if(k1.getIndex().getKey() == Index::KEY_EQUALITY) {
			IndexEntry::SharedPtr ie(new IndexEntry());
			...
			vector<IndexEntry::SharedPtr> tmpStore;
			tmpStore.reserve(256);

			IndexEntry::SharedPtr ie(new IndexEntry());
			err = cursor->first(*ie); // could throw
			if(err == 0 && ie->getDocID() != 0) {
				IndexEntry::SharedPtr last = ie;
