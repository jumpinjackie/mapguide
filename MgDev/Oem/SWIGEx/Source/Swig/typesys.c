...
char cvsroot_typesys_c[] = "$Header: /cvsroot/SWIG/Source/Swig/typesys.c,v 1.10 2003/11/26 20:56:44 beazley Exp $";
...
extern int enableRuntimeTypeChecking;
...
SwigType_emit_type_table(File *f_forward, File *f_table) {
  ...
  {
	Printf(f_table,"\nclass ClsInfo\n{\npublic:\n\tClsInfo(swig_type_info* ti, zend_class_entry* cls) { this->pTypeInfo = ti; this->pCls = cls; }\n\n\tswig_type_info*\t\tpTypeInfo;\n\tzend_class_entry*\tpCls;\n};\n");
	Printf(f_table,"\n#include <map>");
	Printf(f_table,"\nusing namespace std;");
	Printf(f_table,"\nstatic map<int, ClsInfo*> classMap;\n\n");
  }
