...
//
// $Id: TestEnvironment.cpp,v 1.31 2006/10/30 17:46:13 bostic Exp $
//
	...
	const std::string &query, DOMNode *contextNode)
{
	...
	XQilla xqilla;
	XQQuery *parsedQuery = xqilla.parse(UTF8ToXMLCh(query).str(), XQilla::XQUERY,
...
void TestEnvironment::addIndexSpecifications(Sequence &result)
{
	...
	for (Sequence::iterator it = result.begin(); it != result.end(); it++) {
		if((*it)->isNode()) {
			const DOMNode *domnode = (const DOMNode*)(*it)->getInterface(Node::gXerces);
			assert(domnode != 0);
					   ...
					   std::string &text)
{
	...
	const DOMNode *n = (const DOMNode*)theNode->getInterface(Node::gXerces);
	assert(n != 0);
...
Sequence TestEnvironment::parseTestInstructions()
{
	...
	const DOMNode *dnode = (const DOMNode*)dseq.first()->getInterface(Node::gXerces);
	assert(dnode != 0);
...
void TestEnvironment::addGroupContext()
{
	...
	const DOMNode *context = (const DOMNode*)result.first()->getInterface(Node::gXerces);
	assert(context != 0);
...
TestEnvironment::TestType TestEnvironment::getTestType()
{
	...
	const DOMNode *n = (const DOMNode*)result.first()->getInterface(Node::gXerces);
	assert(n != 0);
