...
//
// $Id: ResolverUnitTest.cpp,v 1.12 2006/10/30 17:46:13 bostic Exp $
//
...
void ResolverUnitTest::execute()
{
	...
	for (Sequence::iterator it = bases.begin();
	     it != bases.end(); it++) {
		if((*it)->isNode()) {
			const DOMNode *base = (const DOMNode*)(*it)->getInterface(Node::gXerces);
			assert(base != 0);
			...
			for (Sequence::iterator it1 = inputs.begin();
				...
				if((*it1)->isNode()) {
					const DOMNode *input =
						(const DOMNode*)(*it1)->getInterface(Node::gXerces);
					assert(input != 0);
