...
//
// $Id: XQueryTest.cpp,v 1.33 2006/10/30 17:46:14 bostic Exp $
//
...
void XQueryTest::execute()
{
	...
	for (Sequence::iterator it = result.begin();
		...
		if((*it)->isNode()) {
			...
			DOMNode *domnode = (DOMNode*)(*it)->getInterface(Node::gXerces);
			assert(domnode != 0);
			...
			for (Sequence::iterator it2 = result2.begin();
				...
				if((*it2)->isNode()) {
					DOMNode *dn = (DOMNode*)(*it2)->getInterface(Node::gXerces);
					assert(dn != 0);
