...
#
# $Id: test.tcl,v 1.68 2006/10/30 17:46:09 bostic Exp $
#
...
proc run { { id 0 } {env t } {use_nls 0} {index_nodes 0} { nofork 1 } { output_summary 1 } } {
    ...
    set found 0
    set fail 0
    foreach test $test_names($subsystem) {
    ...
    if { ! $found } {
        puts "Error - test group \"$group\" is not defined in \"\$test_names\" (xmlparams.tcl)"
    }
...
proc run_envmethod { test {envtype ""} args storage_type indexnodes} {
    ...
    global dbxml_err
    env_cleanup $testdir
    set oflags " -create -lock_max_locks 10000 -lock_max_lockers 10000"
