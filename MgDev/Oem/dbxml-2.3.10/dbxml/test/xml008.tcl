...
#
# $Id: xml008.tcl,v 1.44.2.2 2007/01/09 22:52:32 gmf Exp $
#
# TEST	xml008
...
proc xml008_2_1 { {env "NULL"} {txnenv 0} {basename $tnum} oargs } {
    ...
    dbxml_error_check_good "$basename: getFlags" $flags $targs
    delete container
    delete db
}
...
proc xml008_3_2 { {env "NULL"} {txnenv 0} {basename $tnum} oargs } {
	...
	dbxml_error_check_good "$basename: getFlags" [$container getFlags] $oargs
	delete container
    } ret
...
proc xml008_8 { {env "NULL"} {txnenv 0} {basename $tnum} oargs } {
    ...
    set queryExp "for \$i in (1 to 100000) return \$i"
    wrap XmlQueryContext qc [$mgr createQueryContext]

    # first let it work
    wrap XmlResults res [$mgr query $queryExp $qc]
    dbxml_error_check_good $basename.8 [$res size] 100000
    delete res
