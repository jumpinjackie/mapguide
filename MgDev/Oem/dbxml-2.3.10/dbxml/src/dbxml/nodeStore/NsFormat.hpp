...
//
// $Id: NsFormat.hpp,v 1.12 2006/11/17 18:18:29 jsnelson Exp $
//
...
public:
	...
	virtual void unmarshalNodeKey(DocID &did, NsNid *nid,
		XER_NS MemoryManager *mmgr, unsigned char *buf,
		bool copyStrings) const = 0;

	virtual int marshalNodeData(const NsNode *node,
		unsigned char *buf, bool count) const = 0;
	virtual NsNode *unmarshalNodeData(XER_NS MemoryManager *mmgr,
		unsigned char *buf, bool adoptBuffer) const = 0;
	...
	static uint32_t unmarshalId(XER_NS MemoryManager *mmgr, const xmlbyte_t *ptr, NsNid *id);
	static uint32_t unmarshalId(const xmlbyte_t *ptr, NsNid *id,
	...
	static uint32_t unmarshText(unsigned char *ptr, nsText_t *text,
				    xmlbyte_t **endPP, bool copyStrings);
...
inline int NsFormat::unmarshalMAPMToTemporary(const xmlbyte_t *buf, M_APM_struct &tmp, Numeric::State *state)
{
	...
	if(tmp.m_apm_sign == 0) {
		...
		ptr += NsFormat::unmarshalInt(ptr, &tmp.m_apm_exponent);
		ptr += NsFormat::unmarshalInt(ptr, &tmp.m_apm_datalength);
