...
//
// $Id: NsEvent.cpp,v 1.18 2006/10/30 17:45:57 bostic Exp $
//
...
void
NsEventHandler16::docTypeDecl(const xmlch_t *, uint32_t)
{
	
}

void
NsEventHandler16::startEntity(const xmlch_t *, uint32_t)
{
}

void
NsEventHandler16::endEntity(const xmlch_t *, uint32_t)
{
}
...
NsEventNodeAttrList::prefix(int index) const
{
	...
	if (prefix == NS_NOPREFIX)
		return 0;
	_doc.getStringForID(prefix, _curPrefix);
	return  (const xmlbyte_t *) _curPrefix.get_data();
}
...
NsEventNodeAttrList::uri(int index) const
{
	...
	if (uri == NS_NOURI)
		return 0;
	_doc.getStringForID(uri, _curUri);
	return  (const xmlbyte_t *) _curUri.get_data();
}
