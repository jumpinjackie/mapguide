...
void MgResourceDefinitionManager::DeleteRepository(
    MgResourceIdentifier* resource)
{
    MG_RESOURCE_SERVICE_TRY()

    throw new MgInvalidOperationException(
        L"MgResourceDefinitionManager.DeleteRepository", __LINE__, __WFILE__, NULL, L"", NULL);

    MG_RESOURCE_SERVICE_CATCH_AND_THROW(L"MgResourceDefinitionManager.DeleteRepository")
}
...
void MgResourceDefinitionManager::MoveResource(
    ...
    bool overwrite)
{
    ...
    if (!overwrite && FindResource(destResource->ToString()))
    {
        m_repositoryMan.ThrowDuplicateResourceException(*destResource,
            L"MgResourceDefinitionManager.MoveResource",
            __LINE__, __WFILE__);
    }
    ...
    bool isResourceContentManager = IsResourceContentManager();
    XmlManager& xmlManager = m_container.getManager();
    XmlQueryContext queryContext = xmlManager.createQueryContext();
    XmlResults results = IsTransacted() ?
        xmlManager.query(GetXmlTxn(), query, queryContext, 0) :
    ...
    if (0 == results.size())
    {
        if (isResourceContentManager && sourceResourceIsFolder)
        {
            return;
        }
        else
    ...
    XmlUpdateContext updateContext = xmlManager.createUpdateContext();
    const size_t length = sourcePathname.length();
    ...
    while (results.next(xmlValue))
    {
        ...
        MgResourceIdentifier currResource(MgUtil::MultiByteToWideChar(
            currPathname));
        ...
        if (sourceResourceIsFolder)
        {
            currPathname.replace(0, length, destPathname);
        ...
        if (overwrite)
        {
            ...
            if (GetDocument(currPathname, oldDoc))
            {
                if (!isResourceContentManager)
                {
                    oldDoc.getMetaData(
                        MgResourceInfo::sm_metadataUri,
    ...
    m_repositoryMan.UpdateDateModifiedResourceSet(sourceResource->GetFullPath(true));
    m_repositoryMan.UpdateDateModifiedResourceSet(destResource->GetFullPath(true));

    MG_RESOURCE_SERVICE_CATCH_AND_THROW(L"MgResourceDefinitionManager.MoveResource")
}
...
void MgResourceDefinitionManager::CopyResource(
    MgResourceIdentifier* sourceResource, MgResourceIdentifier* destResource,
    bool overwrite)
{
    assert(NULL != sourceResource && NULL != destResource);
    ...
    {
        m_repositoryMan.ThrowDuplicateResourceException(*destResource,
            L"MgResourceDefinitionManager.CopyResource",
            __LINE__, __WFILE__);
    }
    ...
    if (NULL == appRepositoryManager)
    {
        throw new MgInvalidCastException(
            L"MgResourceDefinitionManager.CopyResource", __LINE__, __WFILE__, NULL, L"", NULL);
    }

    string owner;
    MgUtil::WideCharToMultiByte(appRepositoryManager->m_currUserInfo->GetUserName(), owner);
    ...
    while (results.next(xmlValue))
    {
        ...
        CheckPermission(currResource, MgResourcePermission::ReadOnly);
        ...
        XmlDocument newDoc = xmlManager.createDocument();
        ...
        if (isResourceContentManager)
        {
            ...
            if (NULL != appRepositoryManager
                && currDoc.getMetaData(MgResourceInfo::sm_metadataUri,
                    ...
                    tagValue))
            {
                ...
                appRepositoryManager->CopyResourceData(sourceResourceTags,
    ...
    MG_RESOURCE_SERVICE_CATCH_AND_THROW(L"MgResourceDefinitionManager.CopyResource")
}
