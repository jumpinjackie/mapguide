...
public:
    ...
    MgServerSqlProcessor();
    MgServerSqlProcessor(MgServerSqlDataReader* sqlReader);
    ~MgServerSqlProcessor();

    MgBatchPropertyCollection* GetRows(INT32 count = 0); // If zero means feature all records
    MgByteReader* GetLOBFromSqlDataReader(CREFSTRING propName, MgServerSqlDataReader* sqlReader);
...
private:
    ...
    MgServerSqlDataReader*               m_sqlDataReader;
