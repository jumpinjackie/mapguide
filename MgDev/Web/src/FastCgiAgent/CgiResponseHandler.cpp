...
void CgiResponseHandler::SendResponse(MgHttpResponse* response)
{
    ...
    if (status != 200)
    {
        ...
        if (statusMessage == CgiStrings::FailedAuth1 ||
            statusMessage == CgiStrings::FailedAuth2)
        {
            RequestAuth();
        }
        else
        {
            ...
            printf(CgiStrings::StatusHeader, status, MG_WCHAR_TO_CHAR(statusMessage));
            printf(CgiStrings::ContentTypeHeader, CgiStrings::TextHtml, "");
            printf("\r\n"
                "<html>\n<head>\n"
                "<title>%s</title>\n"
    ...
    else
    {
        ...
        if (contentType.length() > 0)
        {
            ...
            {
                charSet = CgiStrings::Utf8Text;
            }
            printf(CgiStrings::ContentTypeHeader, MG_WCHAR_TO_CHAR(contentType), charSet.c_str());
        }
        else
        {
            printf(CgiStrings::ContentTypeHeader, CgiStrings::TextPlain, CgiStrings::Utf8Text);
        }
        ...
        if (stringVal.length() > 0)
        {
            ...
            printf(CgiStrings::ContentLengthHeader, utf8.length());
            printf("\r\n%s",utf8.c_str());
        }
        else if (outputReader != NULL)
        {
            INT64 outLen = outputReader->GetLength();
            printf(CgiStrings::ContentLengthHeader,(INT32)outLen);
            printf("\r\n");
        ...
        {
            printf(CgiStrings::ContentLengthHeader, 0);
            printf("\r\n");
        }
    }
...
void CgiResponseHandler::SendError(MgException* e)
{
    ...
    printf(CgiStrings::StatusHeader, 559, MG_WCHAR_TO_CHAR(statusMessage));
    printf(CgiStrings::ContentTypeHeader, CgiStrings::TextHtml, CgiStrings::Utf8Text);
    printf("\r\n"
        "<html>\n<head>\n"
        "<title>%s</title>\n"
        "<meta http-equiv=\"Content-Type\" content=\"text/html; charset=utf-8\">\n"
...
void CgiResponseHandler::RequestAuth()
{
    ...
    printf(CgiStrings::UnauthorizedHeader);
    printf(CgiStrings::WWWAuth, PRODUCT_NAME);
    string errorMsg = "You must enter a valid login ID and password to access this site\r\n";
    printf(CgiStrings::ContentTypeHeader, CgiStrings::TextPlain, CgiStrings::Utf8Text);
    printf(CgiStrings::ContentLengthHeader, errorMsg.length());
    printf("\r\n%s",errorMsg.c_str());
}
