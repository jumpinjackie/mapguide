...
void MgHttpEnumerateResourceData::Execute(MgHttpResponse& hResponse)
{
    ...
    if (m_format != MgMimeType::Xml && m_format != MgMimeType::Json)
    {
        ...
        throw new MgInvalidFormatException(L"MgHttpEnumerateResourceData::Execute",
    ...
    // Run API command
    Ptr<MgByteReader> byteReaderResult = mgprService->EnumerateResourceData(&mgrIdentifier, m_format);
