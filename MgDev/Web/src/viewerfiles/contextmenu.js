...
function PopupMouseDown(e)
{
    ...
    if(menuObj != null)
    {
       var state = firefox? tgt.attributes["state"].nodeValue: tgt.state;
       if(!(!state || state == "false"))
            executeOption(menuObj, firefox? tgt.attributes["name"].nodeValue: tgt.name);
        return true;
    }
    else
        return false;
...
function showPopup(menuObj, optimizeWidth, x, y)
{
    ...
    for(var i=1; ; i++)
    {
        ...
        if(name.indexOf("Sub:") != 0)
        {
            var state = firefox? obj.attributes["state"].nodeValue: obj.state;
            var enabled = true;
            if(menuObj.owner)
            {
                try
