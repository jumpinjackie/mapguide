...
void MgXmlJsonConvert::ToJson(const string &xmlString, string &jsonString)
{
    ...
    string nodeName;
    UnicodeString::WideCharToMultiByte(X2W(root->getNodeName()), nodeName);
...
void MgXmlJsonConvert::XmlToJsonNode(DOMNode *node)
{
    ...
    if (node->hasAttributes())
    {
        ...
        for (int i = 0; i < attributeCount; i++)
        {
            ...
            UnicodeString::WideCharToMultiByte(X2W(attribute->getNodeName()), nodeName);
            UnicodeString::WideCharToMultiByte(X2W(attribute->getNodeValue()), nodeValue);
    ...
    while (childNode != NULL)
    {
        ...
        UnicodeString::WideCharToMultiByte(X2W(childNode->getNodeName()), nodeName);

        if (childNodeType == DOMNode::TEXT_NODE)
        {
            string textContent;
            UnicodeString::WideCharToMultiByte(X2W(childNode->getTextContent()), textContent);
...
void MgXmlJsonConvert::ProcessObjectNode(DOMNode *node)
{
    ...
    string nodeName;
    UnicodeString::WideCharToMultiByte(X2W(node->getNodeName()), nodeName);
    if (nodeType == DOMNode::TEXT_NODE)
    {
        string textContent;
        UnicodeString::WideCharToMultiByte(X2W(node->getTextContent()), textContent);
