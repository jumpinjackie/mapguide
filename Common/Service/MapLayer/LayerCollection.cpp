...
//
MgLayerCollection::MgLayerCollection(MgMap* owner)
{
...
//
MgLayer* MgLayerCollection::GetItem(INT32 index)
{
    //returned value is addref'd by m_layers base class
    return (MgLayer*)m_layers->GetItem(index);
}
...
MgLayer* MgLayerCollection::GetItem(CREFSTRING name)
{
    ...
    return (MgLayer*)m_layers->GetItem(name);
}

// Sets the item in the collection at the specified index to the specified value
// Throws an out of range exception if the index is out of range.
//
void MgLayerCollection::SetItem(INT32 index, MgLayer* value)
{
    //notify the removal of the current layer at this position
    Ptr<MgLayer> prevLayer = (MgLayer*)m_layers->GetItem(index);
    if (m_owner)
...
//
void MgLayerCollection::Add(MgLayer* value)
{
    //calculate and set the zorder for the new layer
    Ptr<MgLayer> prevLayer = (m_layers->GetCount() == 0? NULL: (MgLayer*)m_layers->GetItem(m_layers->GetCount() - 1));
...
//
void MgLayerCollection::Insert(INT32 index, MgLayer* value)
{
    ...
    {
        zOrderLow = 0;
        layer = m_layers->GetCount() > 0? (MgLayer*)m_layers->GetItem(index): NULL;
        if(layer != NULL)
    ...
    {
        layer = (MgLayer*)m_layers->GetItem(index - 1);
        zOrderLow = layer->GetDisplayOrder();
        layer = m_layers->GetCount() > index? (MgLayer*)m_layers->GetItem(index): NULL;
...
{
    Ptr<MgLayer> layer = SAFE_ADDREF(value);
...
{
    Ptr<MgLayer> layer = (MgLayer*)m_layers->GetItem(index);
...
//
MgMap* MgLayerCollection::GetMap()
{
    return m_owner;
}
