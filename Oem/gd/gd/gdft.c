...
/* EAM DEBUG */
	...
	xshow vector */
	  ...
	  if (! xshow_alloc) {
		xshow_alloc = 10;
		strex->xshow = (float*)malloc(xshow_alloc * sizeof(float));
		xshow_pos = 0;
	  } 
	  else if (xshow_pos + 2 > xshow_alloc) {
		xshow_alloc += 10;
		strex->xshow = realloc(strex->xshow, xshow_alloc * sizeof(float));
	}

      /* TIS - store as numbers instead of strings */
      strex->xshow[xshow_pos++] = (float)(penf.x - oldpenf.x) * hdpi / (64 * METRIC_RES);
      /*
