...
#
# $Id: xml011.tcl,v 1.101 2006/11/09 14:04:35 jsnelson Exp $
#
# TEST	xml011
...
proc xml011 { args } {
    ...
    # W3C Use Cases
    xml011_5 $env $txnenv $cdbenv $basename.5 $oargs
    
    # just-in-time query optimizations
    xml011_6 $env $txnenv $cdbenv $basename.6 $oargs
    
    # Various interesting query cases
    xml011_7 $env $txnenv $cdbenv $basename.7 $oargs
...
#  xml_cleanup [file join $testdir "a"] $env
  ...
  set var_value [$context getVariableValues "foo"]
  ...
  # Check the variable value
  set var_value1 [$context getVariableValues "foo1"]
  set var_value2 [$context getVariableValues "foo2"]
  set var_value3 [$context getVariableValues "foo3"]
...
proc xml011_1_3 { env txnenv cdbenv basename oargs } {
    ...
    wrap XmlQueryContext context [$db createQueryContext]
    $context setReturnType $XmlQueryContext_DeadValues
    $context setEvaluationType $XmlQueryContext_Eager
    assertQueryReturnsValue $testid $db $container {/a/b} \
...
# Preparation and execution of XQuery expressions (W3C Use Cases)
proc xml011_5 { {env "NULL"} {txnenv 0} {cdbenv ""} {basename $tnum} oargs } {
    puts "\txml011.5: Query Processor - XQuery (W3C Use Cases)"
    source ./include.tcl

    xml011_run_queries $env $txnenv $cdbenv $basename "document_set_11_5" $oargs
}

# Preparation and execution of generally interesting XQuery expressions
# Add bug-driven test cases to this set by modifying
# document_set_11_7/index.xml
proc xml011_7 { {env "NULL"} {txnenv 0} {cdbenv ""} {basename $tnum} oargs } {
    puts "\txml011.7: Query Processor - XQuery (General Use Cases)"
    source ./include.tcl

    xml011_run_queries $env $txnenv $cdbenv $basename "document_set_11_7" $oargs
}
...
proc xml011_6 { {env "NULL"} {txnenv 0} {cdbenv ""} {basename $tnum} oargs } {
    ...
    lappend return_types $XmlQueryContext_DeadValues
...
proc xml011_6_run { env txnenv basename oargs et rt } {
    ...
    append msg " / "
    if { $rt == $XmlQueryContext_DeadValues } {
        append msg "Dead Values"
    } elseif { $rt == $XmlQueryContext_LiveValues } {
    ...
    if { ! [string match "*<OQPlan>*node-element-equality-string*</OQPlan>*" $qp_constant] } {
        if { ! [string match "*<PresenceQP*node-element-equality-string*>*" $qp_constant] } {
            dbxml_error_check_good "$testid - query plan not optimized" 0 1
        }
    }

    # test - the "non constant" query plan will be raw
    set qp_non_constant [$xqe_nc getQueryPlan]
    dbxml_error_check_good "$testid - query plan not raw" [string match "*<RQPlan>*</RQPlan>*" $qp_non_constant] 1
...
proc convertReturnType { stringtype } {
  ...
  if { [string equal $stringtype DeadValues] } {
    set type $XmlQueryContext_DeadValues
  } elseif { [string equal $stringtype LiveValues] } {
    set type $XmlQueryContext_LiveValues
  }
  return $type
}
