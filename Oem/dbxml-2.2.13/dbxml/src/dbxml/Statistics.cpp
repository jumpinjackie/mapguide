...
//
// $Id: Statistics.cpp,v 1.58 2005/11/22 16:39:49 jsnelson Exp $
//
...
const KeyStatistics &StatisticsReadCache::getKeyStatistics(const Container &container, OperationContext &context, const Key &key)
{
...
void StatisticsReadCache::populateStatistics(OperationContext &context, const StatsMapKey &key)
{
		...
		while(err == 0) {
			if(context.key().get_size() < original.get_size() ||
			   memcmp(original.get_data(), context.key().get_data(), original.get_size()) != 0) {
				...
				tmpKey.key.setThisFromDbt(context.key());
