...
//
// $Id: Statistics.hpp,v 1.30 2005/07/13 13:54:59 jsnelson Exp $
//
...
public:
	...
	const KeyStatistics &getKeyStatistics(const Container &container, OperationContext &context, const Key &key);
	double getPercentage(const Container &container, OperationContext &context, SecondaryDatabase::Operation op1, Key k1,
...
private:
	...
	struct PercentageMapKey {
		PercentageMapKey(const Container *c, SecondaryDatabase::Operation op1, const Key &k1, SecondaryDatabase::Operation op2, const Key &k2)
	...
	void populateStatistics(OperationContext &context, const StatsMapKey &key);
