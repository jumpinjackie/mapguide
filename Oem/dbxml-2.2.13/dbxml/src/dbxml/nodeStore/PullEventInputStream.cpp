...
//
// $Id: PullEventInputStream.cpp,v 1.1 2005/05/25 16:02:50 jsnelson Exp $
//
...
PullEventInputStream::PullEventInputStream(NsPullEventSource8 *events)
	: events_(events),
	  cursorPos_(0),
	  bufferOffset_(0),
	  toFill_(0),
...
{
	unsigned int originalCursorPos = cursorPos_;
	...
	return cursorPos_ - originalCursorPos;
}
...
void PullEventInputStream::write(const xmlbyte_t *data, uint32_t len)
{
	if(len <= bytesLeft_) {
