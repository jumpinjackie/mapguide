...
//
// $Id: ConfigurationDatabase.cpp,v 1.24 2005/11/22 16:39:48 jsnelson Exp $
//
	  ...
	  creatingTxn_(txn)
{
	...
	try {
		...
		// a create, it won't be there.
		if(err == 0)
			type = checkContainerType(txn, type, rdonly);
		...
		if(e.get_errno() == EEXIST) {
			throw XmlException(
				XmlException::CONTAINER_EXISTS, e.what());
		} else {
...
int ConfigurationDatabase::generateID(Transaction *txn, ID &newId)
{
	int err = 0;
	db_seq_t newValue = 0;
	u_int32_t flags = 0;
				    ...
				    int old_version, int current_version)
{
	...
	if (current_version == VERSION_22) {
		// 1. create sequence database, and move
		// 2. move sequence record from configuration db
		// 3. upgrade version to current_version
		int err;
		...
		if (err == 0) {
			...
			if (err == 0) {
				err = seqDatabase.put(0, &seqKey, &datadbt, 0);
				if (err == 0)
