...
//
// $Id: Cursor.cpp,v 1.60 2005/11/22 16:39:48 jsnelson Exp $
//
...
IndexCursor::IndexCursor(IndexDatabase &db, Transaction *txn, bool initBulk)
	: cursor_(db, txn, CURSOR_READ,
		  ((db.getEnvironment()->get_DB_ENV()->open_flags & DB_INIT_LOCK) ? DB_DEGREE_2 : 0)),
	  done_(false)
{
	if (initBulk) {
...
int IndexCursor::nextEntries(u_int32_t flags)
{
	...
	while(err == DB_BUFFER_SMALL) {
		...
		delete [] (char*)bulk_.get_data();
		bulk_.set_data(new char[bulk_.get_size() / sizeof(char)]);
		bulk_.set_ulen(bulk_.get_size() / sizeof(char));
...
int InequalityIndexCursor::next(IndexEntry &ie)
{
	...
	case DbWrapper::LTX: {
		if(syntax_->bt_compare(0, &tmpKey_, &key_) >= 0) {
			done_ = true;
			ie.reset();
			return 0;			
		}
...
int ReverseInequalityIndexCursor::findLastValue(Dbt &key, bool excl)
{
	...
	if (err != 0) {
		...
		if(syntax_->bt_compare(0, &tmpKey_, &key) == 0) {
